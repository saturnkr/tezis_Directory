...
lang.extend(WireIt.WiringEditor, WireIt.BaseEditor, {
	...
	try {
      ...
      div.appendChild( WireIt.cn('span', null, null, module.label || module.name) );
	...
	addModule: function(module, pos) {
	    try {
	       ...
	       containerConfig.title = module.label || module.name;
           containerConfig.moduleName = module.name;
	       var temp = this;
		   ...
		   if(lang.isArray(wiring.modules)) {
		      ...
		      for(i = 0 ; i < wiring.modules.length ; i++) {
		         ...
		         if(this.modulesByName[m.name]) {
                    var moduleDef = this.modulesByName[m.name];
		            var baseContainerConfig = this.modulesByName[m.name].container;
		            YAHOO.lang.augmentObject(m.config, baseContainerConfig); 

		            m.config.title = moduleDef.label || moduleDef.name;
                    m.config.moduleName = moduleDef.name;
	...
	getValue: function() {
	  ...
	  for( i = 0 ; i < this.layer.containers.length ; i++) {
	     obj.modules.push( {name: this.layer.containers[i].moduleName, value: this.layer.containers[i].getValue(), config: this.layer.containers[i].getConfig()});
	  }
