-- $Id: 01-20-initDefaultTaskman.sql 20627 2015-07-06 09:41:19Z mishunin $
-- Description:
...
update WF_PROC_ROLE set name = 'Initiator', role_id = (select id from SEC_ROLE where name = 'task_initiator'),assign_to_creator='TRUE' where code = '10-Initiator' and proc_id = (select id from WF_PROC where jbpm_process_key = 'TaskManagement') and updated_by is null;
update WF_PROC_ROLE set name = 'Executor', role_id = (select id from SEC_ROLE where name = 'task_executor'), order_filling_type = 'P', is_multi_user = 'true' where code = '20-Executor' and proc_id = (select id from WF_PROC where jbpm_process_key = 'TaskManagement') and updated_by is null;
...
update WF_PROC_ROLE set name = 'Observer', role_id = (select id from SEC_ROLE where name = 'task_observer'), order_filling_type = 'P', is_multi_user = 'true' where code = '90-Observer' and proc_id = (select id from WF_PROC where jbpm_process_key = 'TaskManagement') and updated_by is null;
...
if(cnt = 0) then
insert into TM_PROJECT_GROUP (id, name, create_ts, created_by, version) values ('b875aded-88e7-4090-9f81-a1a5fac75192', 'Basic project', '2010-10-28 00:00:00.00', 'admin', 1);
end if;

cnt = (select count(CATEGORY_ID) from TM_TASK_TYPE where CATEGORY_ID = '836d02a0-68a9-4aa5-b11a-08e9b32c589b');
if(cnt = 0) then
insert into SYS_CATEGORY (ID, NAME, ENTITY_TYPE, IS_DEFAULT, CREATE_TS, CREATED_BY, VERSION, DISCRIMINATOR)
values ('836d02a0-68a9-4aa5-b11a-08e9b32c589b', 'Basic', 'tm$Task', false, now(), USER, 1, 2);
insert into TM_TASK_TYPE (CATEGORY_ID, create_ts, created_by, version, category_attrs_place, tab_name) values ('836d02a0-68a9-4aa5-b11a-08e9b32c589b', '2010-10-28 00:00:00.00', 'admin', 1, 1, 'Доп. поля');
end if;
