package com.company.tezisdirectory.core.app;
...
@ManagedBean("tezis_Directory_ExtDeployer")
public class ExtDeployer extends AbstractDeployer implements ExtDeployerMBean {
    ...
    protected void checkForFirstInit() {
        ...
        try {
            ...
            List<Config> configs = em.createQuery("select c from sys$Config c where c.name='tezis_Directory.initDefaultInFirstStart'",
                ...
                if (StringUtils.isNotBlank(edition) && Integer.parseInt(edition) > 1) {
                    ...
                    if (initDefaultInFirstStart == null) {
                        try {
                            ...
                            config.setName("tezis_Directory.initDefaultInFirstStart");
                        ...
                        try {
                            login();
                            em.createQuery("update sys$Config c set c.value='false' where c.name='tezis_Directory.initDefaultInFirstStart'",
    ...
    protected void executeInitScripts() {
        ...
        File folderDB = new File(dbDirPath + "/50-tezis_Directory/init/" + dbmsType);
